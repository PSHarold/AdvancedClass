...
class ResetPasswordConfirmTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        emailTextField.placeholder = TeacherAuthenticationHelper.defaultHelper.emailToVerify!
    }

    override func tableView(tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        if section == 0{
            return "请补全邮箱地址\(TeacherAuthenticationHelper.defaultHelper.emailToVerify!)以验证账户"
        }
        return nil
    }
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        ...
        self.showHudIndeterminate("正在验证")
        TeacherAuthenticationHelper.defaultHelper.resetPasswordConfirmEmail(self.emailTextField.text!){
            [unowned self]
            error in
            if let error = error{
