...
class TeacherAuthenticationHelper {
    ...
    func requestToResetPassword(userId: String, completionHandler: ResponseMessageHandler){
        ...
        self.getResponsePOST(RequestType.RESET_PASSWORD_GET_EMAIL, parameters: ["user_id": userId, "role": ROLE_FOR_TEACHER]){
            [unowned self]
            error, json in
            if error == nil{
    ...
    func resetPasswordConfirmEmail(email: String, completionHandler: ResponseMessageHandler){
        self.getResponsePOST(RequestType.RESET_PASSWORD_CONFIRM_EMAIL,  parameters: ["user_id": self.userIdToReset!, "role": ROLE_FOR_TEACHER, "email": email]){
            error, json in
            completionHandler(error: error)
        }
    }
